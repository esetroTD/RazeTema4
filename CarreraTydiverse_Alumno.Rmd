...
mydf <- read.csv('~/1er CURSO GRADO CIENCIA DE DATOS UV/TRATAMIENTO DE LOS DATOS/Tema4. Manejo de datos-20250312/RazeTema4/data/2024-03-12.Rdata')

set.seed(seed=14032024)
N<-200000
I<-sample(1:nrow(mydf),size=N)
mydf<-mydf[I,]
# Guardamos en formato binario, la primera vez que cargamos
save(file = '~/1er CURSO GRADO CIENCIA DE DATOS UV/TRATAMIENTO DE LOS DATOS/Tema4. Manejo de datos-20250312/RazeTema4/data/2024-03-12.Rdata',list=c('mydf'))
```
...
load(file ='~/1er CURSO GRADO CIENCIA DE DATOS UV/TRATAMIENTO DE LOS DATOS/Tema4. Manejo de datos-20250312/RazeTema4/data/2024-03-12.Rdata')
dim(mydf)
head(mydf)
...
```{r}
library(dplyr)
cran_selected <- cran[, c("ip_id", "package", "country")]
```
...
```{r}
top_counts<- cran%>%count(package)%>%filter(n>quantile(n,probs=0.99))
...
```{r}
top_counts_sorted<- cran%>%count(package)%>%filter(n>quantile(n,0.99))%>% arrange(desc(n))
...
```{r}
top_unique_sorted <- cran %>%group_by(package) %>%summarise(unique = n_distinct(ip_id)) %>%  filter(unique > quantile(unique, 0.99)) %>% arrange(desc(unique)) 
...
```{r}
cran%>%mutate(size_mb=size/2^20)%>%print()
...
```{r}
cran %>% mutate(size_mb=size/2^20)%>%filter(size <=0.5)
...
```{r}
cran %>% mutate(size_mb=size/2^20)%>%filter(size <=0.5)%>%arrange(desc(size_mb))%>%print()
...
tidy_students <- students %>% pivot_longer(cols = c(male, female),names_to = "sex", values_to = "count") 
